...
$sql = db_query("SELECT project_id, app_title, creation_time, project_pi_firstname, project_pi_lastname, project_pi_email,last_logged_event FROM redcap_projects WHERE status='0' AND app_title NOT LIKE '%test%' AND app_title NOT LIKE '%copy%' AND app_title NOT LIKE '%example%' AND app_title NOT LIKE '%longitudinal%' ORDER BY creation_time DESC");
$filename = "Consortium_Projects_Development_".$report_name.date("Y-m-d_H-m-s").".csv";
